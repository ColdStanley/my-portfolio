...
// POST: Upload column to marketplace (demo version)
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = demoMarketplaceItems.find(item => item.content_hash === contentHash)
    if (existing) {
      return NextResponse.json(
    ...
    const newItem = {
      ...
      data: { ...cleanData, original_author_id: 'demo-user-current' },
      content_hash: contentHash,
      author_id: 'demo-user-current',
      author_name: 'You',
      downloads: 0,
      tags: tags || [],
    ...
    demoMarketplaceItems.unshift(newItem) // Add to beginning
...
// GET: Fetch marketplace items (demo version)
export async function GET(request: NextRequest) {
  try {
    ...
    // Sort items
    let sortedItems = [...demoMarketplaceItems]
    
    if (sortBy === 'downloads') {
      sortedItems.sort((a, b) => order === 'desc' ? b.downloads - a.downloads : a.downloads - b.downloads)
    } else if (sortBy === 'name') {
      sortedItems.sort((a, b) => order === 'desc' 
    ...
    // Remove data field for list view
    const itemsWithoutData = paginatedItems.map(({ data, ...item }) => item)
