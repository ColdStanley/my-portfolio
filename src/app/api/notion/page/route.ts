...
export async function GET(request: Request) {
  ...
  try {
      ...
      blocks.map(async (block) => {
        switch (block.type) {
          ...
          case 'image': {
            ...
            return `<img src="${src}" alt="${alt}" style="max-width:100%; border-radius:8px; box-shadow:0 2px 6px rgba(0,0,0,0.1); margin:20px 0;" />`
          }
          ...
          case 'embed': {
            ...
            if (url.includes('youtube.com') || url.includes('youtu.be')) {
              ...
              return `<div class="video-container"><iframe width="560" height="315" src="${embedUrl}" frameborder="0" allowfullscreen></iframe></div>`
            }
    ...
    return NextResponse.json({ html: html.join('\n') })
  } catch (error) {
