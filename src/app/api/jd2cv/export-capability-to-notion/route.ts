...
export async function POST(request: NextRequest) {
  try {
    const { pageId, capabilityIndex, capabilityValue, keywords = [] } = await request.json()

    if (!pageId || !capabilityIndex || capabilityValue === undefined) {
      return NextResponse.json(
        { error: 'Missing required fields: pageId, capabilityIndex, capabilityValue' },
        { status: 400 }
      )
    }

    console.log(`Exporting capability ${capabilityIndex} to Notion with keywords:`, keywords)
    ...
    const calloutTitle = `Role Expectation ${capabilityIndex}`
    ...
    // Create new capability callout with bold keywords
    try {
            ...
            callout: {
              ...
              children: [
                ...
                },
                {
                  ...
                  type: 'paragraph',
                  paragraph: {
                    rich_text: createRichTextWithBoldKeywords(capabilityValue, keywords),
                  },
                },
              ],
            },
      ...
      return NextResponse.json({ 
        success: true, 
        message: `Exported capability ${capabilityIndex} to Notion with ${keywords.length} keywords` 
      })
    } catch (calloutError) {
    ...
    console.error('Error exporting capability to Notion:', error)
    return NextResponse.json(
      { error: 'Failed to export capability to Notion' },
      { status: 500 }
    )
  }
}