...
import BilibiliPluginPanel from './components/BilibiliPluginPanel'
import BilibiliSingleVideoPanel from './components/BilibiliSingleVideoPanel'

interface BilibiliUploaderPageProps {
  params: Promise<{
    uploader: string
  }>
}

export default function BilibiliUploaderPage({ params }: BilibiliUploaderPageProps) {
  const { uploader } = use(params)
  const [activeTab, setActiveTab] = useState('plugin-version')
  ...
  const tabs = [
    { key: 'plugin-version', label: 'UP主分析' },
    { key: 'single-video', label: '视频分析' },
  ]
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-purple-50/30">
      <>
        ...
        >
          ...
          <div className="flex justify-between items-center p-4 border-b border-gray-100">
            <h2 className="text-sm font-medium text-gray-800">{uploader} 工作台</h2>
      ...
      <div className="flex">
        ...
        <div className="flex-1 ml-[280px] p-6">
          {activeTab === 'plugin-version' && (
            <BilibiliPluginPanel 
              data={uploaderData}
              onDataChange={setUploaderData}
            />
          )}
          
          {activeTab === 'single-video' && (
            <BilibiliSingleVideoPanel 
              data={videoData}
              onDataChange={setVideoData}
            />
          )}
        </div>
      </div>
