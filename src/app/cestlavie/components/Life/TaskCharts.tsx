...
function TaskQuadrantChart({ tasks }: { tasks: TaskRecord[] }) {
  const chartData = useMemo(() => {
    ...
    return { entries, total, quadrantColors, getQuadrantLabel }
  }, [tasks])
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="flex h-3 bg-gray-100 rounded-full overflow-hidden">
            {chartData.entries.map(([quadrant, count]) => {
              const percentage = (count / chartData.total) * 100
              return (
                <div
                  key={quadrant}
                  ...
                  style={{
                    width: `${percentage}%`,
                    backgroundColor: chartData.quadrantColors[quadrant] || '#9333ea'
                  }}
                  title={`${chartData.getQuadrantLabel(quadrant)}: ${count} tasks (${percentage.toFixed(1)}%)`}
                />
              )
            })}
          </div>
      ...
      <div className="mt-2 space-y-1">
        {chartData.entries.map(([quadrant, count]) => (
          <div key={quadrant} className="flex items-center gap-2">
            <div 
              className="w-3 h-3 rounded-full flex-shrink-0"
              style={{ backgroundColor: chartData.quadrantColors[quadrant] || '#9333ea' }}
            />
            <span className="text-xs text-gray-600 flex-1" title={quadrant}>
              {chartData.getQuadrantLabel(quadrant)}
            </span>
            <span className="text-xs text-gray-400 font-medium">({count})</span>
          </div>
        ))}
